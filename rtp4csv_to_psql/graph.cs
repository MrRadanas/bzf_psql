...
{ 
    class graph
    {
        ...
        static void Main(string[] args)
        {
          ...
          /*try
            {
                StreamWriter flogfile = System.IO.File.CreateText(@"%USERPROFILE%\logfile_" + DateTime.Now.Day.ToString() + ".txt");
                flogfile.Close();

            }
            catch { Console.WriteLine("Ошибка создания лог-файла!"); }*/
            ...
            if (args.Length != 0)
            {
                ...
                else { Console.WriteLine("Неверно составлен config файл!"); Console.ReadKey(); return; }

            } else 
            {  
                System.Console.WriteLine("Используется Config.txt в каталоге программы;\nРучной режим:\n");
                 try
                 {
        ...
        static void ExecuteTransactionIndiv(string pg_query)
        {
            ...
            {                
                ...
                OdbcTransaction transaction = null;
                command.Connection = connection;
             //   LogfileInsert("Выполняется, запись данных в БД", next++, pathconf);
              //  LogfileInsert(pg_query, next++, pathconf);
                try
                {
                    connection.Open();
                    transaction = connection.BeginTransaction();
                    ...
                    transaction.Commit();
                    connection.Close();
               //     LogfileInsert("Выполнено", next++, pathconf);
                }
                catch (Exception ex)
                {
                    Console.WriteLine(ex.Message);
               //     LogfileInsert("Исключение.Произведен откат", next++, pathconf);
                    try
                    {
                        transaction.Rollback();
                    }
                    ...
                    {
               //         LogfileInsert("Исключение. НЕ произведен откат!", next++, pathconf); return;
                    } return;
                }
            }
        ...
        /*
        static void gou_exe(string pathconf,int av, int bv)
        {
            try
            {
                string pg_odbc;
                string[] tablenamepsql = new string[22] { "gou11raref", "gou11temp", "gou12graph", "gou13raref", "gou13temp", "gou1level", 
                                                      "gou21raref", "gou21temp", "gou22graph", "gou23raref", "gou23temp", "gou2level", 
                                                      "gou41raref", "gou41temp", "gou42graph", "gou43raref", "gou43temp", "gou4level", 
                                                      "gou31temp",  "gou33temp", "gou32graph", "gou3level"};
                string[][] fildname = new string[22][] { 
            new string[4] { "c401","c402","c404","c403"},
            new string[8] { "cai101","cai102","cai103","cai104","cai105","cai107","cai108","cai109"},
            new string[8] { "cai117","cai118","cai119","cai120","cai122","cai123","cai125","cai126"},
            new string[5] { "c401","c402","c403","c404","cai113"},
            new string[9] { "cai101","cai102","cai103","cai104","cai105","cai106","cai107","cai108","cai109"},
            new string[2] {"cai115","cai116"},

            new string[5] { "c401","c402","c404","c403","cai113"},
            new string[8] { "G1_cai105","cai101","cai102","cai103","cai104","cai107","cai108","cai109"},
            new string[8] { "cai117","cai118","cai119","cai120","cai122","cai123","cai125","cai126"},
            new string[5] { "c401" ,"c402" ,"c403" ,"c404" ,"cai113"},
            new string[8] { "cai101","cai102","cai103","cai104","cai105","cai107","cai108","cai109"},
            new string[2] {"cai115","cai116"},
            ...
            new string[5] { "c401","c402","c404","c403","cai113"},
            new string[9] { "cai105","cai101","cai102","cai103","cai104","cai106","cai107","cai108","cai109"},
            new string[8] { "cai117","cai118","cai119","cai120","cai122","cai123","cai125","cai126"},
            new string[5] { "c401" ,"c402" ,"c403" ,"c404" ,"cai113"},
            new string[8] { "cai101","cai102","cai103","cai104","cai105","cai107","cai108","cai109"},
            new string[2] {"cai115","cai116"},
               
            new string[5] { "cai129","cai130","cai131","cai132","cai137"},
            new string[5] { "cai133","cai134","cai135","cai136","cai138"},
            new string[4] { "cai117","cai118","cai119","cai120"},
            new string[2] {"cai115","cai116"}
            };
                ...
                string[] dbfold = new string[22] { "GOT1_1_", "GOT1_1_", "GOT1_1_", "GOT3_1_", "GOT3_1_", "GOT3_1_", "GOT1_2_", "GOT1_2_", "GOT1_2_", "GOT3_2_", "GOT3_2_", "GOT3_2_", "GOT1_", "GOT1_", "GOT1_", "GOT3_", "GOT3_", "GOT1_", "GOT3_", "GOT3_", "GOT3_", "GOT3_" };

                string[] Dbhost = new string[22];
                for (int i = 0; i < 22; i++)
                { Dbhost[i] = dbfold[i] + year + "_"; }
                string[] iphost = new string[3] { "10.21.1.218", "10.21.1.215", "10.21.1.209" };


                string path = System.IO.File.ReadAllLines(pathconf)[5];
                string pg_query;// = System.IO.File.ReadAllLines(@".\inquery.txt")[4];
                ...
                for (int i = av; i < bv; i++)
                {
                    try
                    {
                        ...
                        for (int y = 0; y < fildname[i].Length; y++)
                        {
                            pg_query = "SELECT time as time_gp, value as value_gp FROM " + fildname[i][y];
                            // pg_query = "SELECT * FROM " + fildname[i][y];
                            conn.Open();
                            ...
                            {
                                ourDataTable.Rows[gi][2] = fildname[i][y];
                            ...
                            File.WriteAllText(path + dbfold[i] + "\\" + fildname[i][y] + ".csv", sb.ToString(), Encoding.UTF8);
                            Console.WriteLine("Иморт графиков для ДО завершен" + fildname[i][y]);
                            ExecuteTransactionIndiv(File.ReadAllLines(@"%USERPROFILE%\logfile_" + DateTime.Now.Day.ToString() + ".txt").Length, "copy asutp." + tablenamepsql[i] + " (time_gp, value_gp, fild_gp, date_gp ) from '/share/import/" + dbfold[i] + "/" + fildname[i][y] + ".csv' DELIMITER ',' CSV null as '' HEADER escape '\\'", pathconf);
                    ...
                    {
                        Console.WriteLine("Ошибка при иморте графиков для GOU3");
                        continue;
                    }
                }
            }
        ...
        */
        static void gou_exe(string pathconf, int av, int bv,int x)
        {
            try
            {
                ...
                System.Net.NetworkInformation.PingReply pingReply = ping.Send("10.21.1.218");
        ...
        static void gou3_exe(string pathconf)
        {
            try
            {
                ...
                for (int y = 0; y < 12; y++)
                {
                    dateday[y] = DateTime.Parse("28.09.15 08:00:00").ToString("yyyy-MM-dd HH:mm:ss");
                }*/
        ...
        static void TehDBF(string pathconf)
        {
           try  {
                ...
                conn.ConnectionString = @"Driver={Microsoft dBase Driver (*.dbf)};DriverID=277; Dbq=" + System.IO.File.ReadAllLines(pathconf)[15] + ";";
                        ...
                        colString[0].DataType = System.Type.GetType("System.DateTime");
                        colString[1].DataType = System.Type.GetType("System.String");
                        colString[2].DataType = System.Type.GetType("System.String");
                        colString[3].DataType = System.Type.GetType("System.String");
                        colString[4].DataType = System.Type.GetType("System.String");
                        colString[5].DataType = System.Type.GetType("System.String");
                        colString[6].DataType = System.Type.GetType("System.String");
                        colString[7].DataType = System.Type.GetType("System.String");
                        colString[8].DataType = System.Type.GetType("System.String");
                        colString[9].DataType = System.Type.GetType("System.String");
                        colString[10].DataType = System.Type.GetType("System.String");
                        colString[11].DataType = System.Type.GetType("System.String");
                        colString[12].DataType = System.Type.GetType("System.String");
                       ...
                       // oCmd.CommandText = "SELECT '" + datein + " '+TIME AS datetimegraph,K1,K2,K3,K4,K5,K6,K7,K8,K9,K10,K11,K12 FROM " + files[files.Length - 1] + " where DATA=Date()-1 union all SELECT '" + datein + " '+TIME AS datetimegraph,K1,K2,K3,K4,K5,K6,K7,K8,K9,K10,K11,K12 FROM " + files[files.Length - 2] + " where DATA=Date()-1 union all SELECT '" + datein + " '+TIME AS datetimegraph,K1,K2,K3,K4,K5,K6,K7,K8,K9,K10,K11,K12 FROM " + files[files.Length - 3] + " where DATA=Date()-1";
            ...
            try
            {
                ...
                for (int ix = 0; ix < idVTI.Length; ix++)
                {
                    ...
                    try
                    {
                        ...
                        // cmd.CommandText = "USE [e6work] GO DECLARE	@return_value int,@idReq int,@Done int SELECT	@idReq = 58874 EXEC	@return_value = [dbo].[ep_AskVTIdata] @Cmd = 'List',@idVTI = 1125,@TimeStart = '25-10-2016 00:00:00',@TimeEnd = '26-10-2016 00:00:00',@ShiftBeg = 3,@ShiftEnd = 3,@idReq = @idReq OUTPUT,@Done = @Done OUTPUT SELECT	@idReq as N'@idReq',@Done as N'@Done' SELECT	'Return Value' = @return_value";
                  ...
                  //  string pg_query;// = System.IO.File.ReadAllLines(@".\inquery.txt")[4];
                    ...
                    cmd.Connection = conn;
                      ...
                      try
                      {
                        ...
                        //  pg_querys[3] = "select Data,Shift,TStart,TStop,Nline,Nfurn,Side,NPock,Quarz,Oreh,OilCoke,Coal,Iron,WChip,Other,BurCoal,SetQz,SetOreh,SetOilCoke,SetCoal,SetIron,SetWChip,SetOther,SetBurCoal from TECHNOLOG where Data = \'" + datedbf.AddDays(-1).ToString("dd/MM/yyyy") + "\' and Data = \'" + datedayon + "\'";  
                          
                       //   pg_query = "select Data,Shift,TStart,TStop,Nline,Nfurn,Side,NPock,Quarz,Oreh,OilCoke,Coal,Iron,WChip,Other,BurCoal,SetQz,SetOreh,SetOilCoke,SetCoal,SetIron,SetWChip,SetOther,SetBurCoal from TECHNOLOG where Data IN (\'" + datedayon + "\')";
                    conn.Open();
        ...
        static void doz_exe(string pathconf)
        {
            try
            {
                //string[] tablenamepsql = new string[8] { "doz_material", "doz_delta", "doz_voltage", "doz_current", "doz_mass", "doz_massvd", "doz_delivvd", "doz_deltavd" };
        ...
        /*
        static void LogfileInsert(string Loglist, int next,string pathconf)
        {
          string pathlog = @"%USERPROFILE%\logfile_" + DateTime.Now.Day.ToString() + ".txt";
            string appendText = Loglist + Environment.NewLine;
            File.AppendAllText(pathlog, appendText);
          ...
          try
            {
              List<string> logh=new List<string>();
               ...
               // DateTime dated = Convert.ToDateTime("31-08-17 08:05:00");
                if (mode == "3sm_auto") { dated = DateTime.Now.AddDays(-1); }
                string dateday = dated.Date.ToString("MM.dd.yy");
                int pday = dated.Day;
                string day;
                string month;
                ...
                float tempv = 0;
                int countflow = 0;
            //     string filecopy = System.IO.File.ReadAllLines(pathconf)[17] + day + "-" + month + "-" + year;
            //     string catalg = System.IO.File.ReadAllLines(pathconf)[19] + day + "-" + month + "-" + year; ;
           /* if (File.Exists(filecopy)) { 
                     File.Copy(filecopy, catalg, true); } else {
                         Console.WriteLine("Нет доступа к файлу!"); return;
                     }*/
                ...
                else { Console.WriteLine("Нет файла:" + fileName); return; }

                if (countflow > 0) { ExecuteTransaction(2, pathconf); } else { logerror("РТП1 Ошибка добавления записей! " + DateTime.Now.ToString()); }
        ...
        { if (File.Exists(@"%USERPROFILE%\Desktop\log_graph.txt")) { File.AppendAllText(@"%USERPROFILE%\Desktop\log_graph.txt", mode, Encoding.UTF8); } else { File.WriteAllText(@"%USERPROFILE%\Desktop\log_graph.txt", mode, Encoding.UTF8); } }

        static void logerror_l(List<string> mode)
        { if (File.Exists(@"%USERPROFILE%\Desktop\log_graph.txt")) { File.AppendAllLines(@"%USERPROFILE%\Desktop\log_graph.txt", mode, Encoding.UTF8); } else { File.WriteAllLines(@"%USERPROFILE%\Desktop\log_graph.txt", mode, Encoding.UTF8); } }
